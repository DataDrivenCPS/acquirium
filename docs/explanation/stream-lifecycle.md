...
graph.
A stream's `metadata` key adds a third piece, in the same graph: the
`hasProperty` edge from its equipment when `entity` is given, and one
`attr:` predicate per leaf of any other key, as
