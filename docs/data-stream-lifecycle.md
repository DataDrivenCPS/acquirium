...
Numeric streams can still contain occasional nonnumeric rows. If a value in a numeric stream cannot be converted to a float, Acquirium stores that row in `timeseries.text_value` rather than rejecting the whole insert. Read APIs expose `value_mode` to choose registered-kind behavior, numeric-only rows, text-only rows, or a coalesced mixed stream. See [`data-api.md`](data-api.md) for the read modes and examples.
