...
| Mode | Behavior |
| --- | --- |
| `registered` | Default. Use the stream's registered `value_kind`: numeric streams return `numeric_value`; text streams return `text_value`. |
...
```python
value_mode="registered"  # [1.0, None, 2.5]
value_mode="numeric"     # [1.0, 2.5]
value_mode="text"        # ["Manual Control"]
value_mode="coalesce"    # ["1.0", "Manual Control", "2.5"]
```

`registered` is the default to preserve existing behavior. Use `coalesce` when
...
```python
query.dataframe(shape="narrow", value_mode="registered")
...
numeric. Text-only streams are registered as text.
