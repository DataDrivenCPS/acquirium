...
class DummyCSVDriver(FileIngestDriver):
    ...
    def _bind_points(self) -> None:
        ...
        for point_uri, column in mapping.items():
            self.declare(safe_stream_name(column), point_uri=point_uri)
    ...
    def read(self, path: Path, cursor: Any) -> tuple[pl.DataFrame, Any]:
        ...
        streams = {c: safe_stream_name(c) for c in df.columns if c not in ("Date", "Time")}
        ...
        return observations, offset + len(df)
