...
from acquirium.Materialization import Transformation, outputs
...
def test_config_app_class_spec_loads_from_its_config_directory(tmp_path: Path):
    ...
    app_file.write_text(
        "from acquirium.Materialization import Transformation, outputs\n"
        "class TemperatureApp(Transformation):\n"
    ...
    assert issubclass(target, Transformation)
...
def test_config_app_registrar_receives_table_options_and_deploys_returned_classes(tmp_path: Path, monkeypatch):
    ...
    class FakeAcquirium:
        ...
        def deploy_transformation(self, transformation, *, parameters=None):
