...
def build(erd_type=None):
    ...
    # TOC (WaterTAP's standard "nonvolatile_toc" constituent) is tracked through
    # pretreatment so organic removal by media/cartridge filtration can be
    # measured. Like tss, it is not carried across the RO translator.
    m.fs.prop_prtrt = prop_ZO.WaterParameterBlock(
        solute_list=["tds", "tss", "nonvolatile_toc"]
    )
...
def set_operating_conditions(m):
    ...
    conc_mass_toc = 0.003 * pyunits.kg / pyunits.m**3  # ~3 mg/L influent TOC
    ...
    m.fs.feed.conc_mass_comp[0, "nonvolatile_toc"].fix(conc_mass_toc)
