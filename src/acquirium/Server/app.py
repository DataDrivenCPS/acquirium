    ...
    try:
        ...
        for s in streams:
            if s.point_uri is None and not s.replace:
                bulk_streams.setdefault((s.source_id, s.cascade), {})[s.ref_name] = s.values
            else:
                individual_streams.append(s)

        for (source_id, cascade), source_streams in bulk_streams.items():
            total += app.state.manager.insert_timeseries_batch(
                source_id, source_streams, cascade=cascade,
            )
