...
def _decoded_source_id(source_id: str) -> str:
    if not source_id.startswith(_ENCODED_SOURCE_PREFIX):
        return source_id
    source_id = source_id[len(_ENCODED_SOURCE_PREFIX):]
    padding = "=" * (-len(source_id) % 4)
    try:
        return urlsafe_b64decode(f"{source_id}{padding}".encode("ascii")).decode("utf-8")
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Invalid source id token: {source_id}") from exc
